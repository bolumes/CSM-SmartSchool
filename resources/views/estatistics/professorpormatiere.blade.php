   ...
   @include('partials.sidebarsettings')
    ...
    <div class="main-content">
        <fieldset style="border-radius: 8px; border: 2px solid blue;">
            <legend style="text-align: center;"><h3 style="text-align: center; color: blue;">PROFESSEUR POUR MATIERE</h3></legend>
...
<div style="overflow-x: auto; margin-bottom: 2rem;">
    <table style="width: 100%; border-collapse: collapse; background-color: #fff; border: 1px solid #ccc; border-radius: 6px;">
        <thead>
            <tr>
                <th style="padding: 12px; background-color: #1c359d; color: white;">Matiere</th>
                <th style="padding: 12px; background-color: #1c359d; color: white;">N° Prof</th>
            </tr>
        </thead>
        <tbody>
            @foreach($estatistics as $estatistic)
                <tr style="transition: 0.3s;" onmouseover="this.style.backgroundColor='#eef2ff'" onmouseout="this.style.backgroundColor=''">
                    <td style="padding: 10px; border: 1px solid #ccc;">{{ $estatistic->matiere_code }}</td>
                    <td style="padding: 10px; border: 1px solid #ccc;">{{ $estatistic->numero_de_professors }}</td>
                </tr>
            @endforeach
        </tbody>
    </table>
...
<!-- Gráfico -->
<div class="chart-container" style="width: 70%; margin: 0 auto;">
    <canvas id="chartjs_matiere"></canvas>
</div>
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        const ctx = document.getElementById('chartjs_matiere').getContext('2d');

        const labels = @json($estatistics->pluck('matiere_code'));
        const data = @json($estatistics->pluck('numero_de_professors'));
        ...
        new Chart(ctx, {
            ...
            data: {
                labels: labels,
                datasets: [{
                    label: 'Número de Professores por Formação',
                    data: data,
                    backgroundColor: [
